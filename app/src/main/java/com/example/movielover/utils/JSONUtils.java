...
public class JSONUtils {
    ...
    // params for pictures according to API docs
    ...
    public static ArrayList<Trailer> getTrailersFromJSON(JSONObject jsonObject) {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                String key = BASE_YOUTUBE_URL + jsonObjectTrailer.getString(KEY_VIDEO_KEY);
    ...
    public static ArrayList<Movie> getMoviesFromJSON(JSONObject jsonObject) {  // p.1
        ArrayList<Movie> result = new ArrayList<>();
        if (jsonObject == null) {    // jsonObject from NetworkUtils
            return result;
        }
        try {
            JSONArray jsonArray = jsonObject.getJSONArray(KEY_RESULTS);
            // getting movies in the cycle
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                // then making an object Movie and adding it to the Movie array
